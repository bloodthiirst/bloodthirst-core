...
{
    internal class BJsonRootMonoBehaviourConverter : BJsonComplexBaseConverter
    {
        ...
        private BJsonRefMonoBehaviourConverter RefMonoBehaviourConverter { get; set; }
        private int MemberCount { get; set; }

        public BJsonRootMonoBehaviourConverter(Type t) : base(t)
        {
        ...
        public override void Initialize()
        {
            ...
            RefMonoBehaviourConverter = new BJsonRefMonoBehaviourConverter();
            RefMonoBehaviourConverter.Provider = Provider;
            RefMonoBehaviourConverter.Initialize();
        ...
        public override void Serialize_NonFormatted_Internal(object instance, StringBuilder jsonBuilder, BJsonContext context, BJsonSettings settings)
        {
            ...
            foreach (KeyValuePair<string, BMemberData> kv in MemberToDataDictionary)
            {
                ...
                if (TypeUtils.IsSubTypeOf(kv.Value.Type, typeof(Component)))
                {
                    cnv = RefMonoBehaviourConverter;
                }

                else if (!settings.HasCustomConverter(kv.Value.Type, out cnv))
                {
        ...
        public override object Deserialize_Internal(ref ParserState<JSONTokenType> parseState, BJsonContext context, BJsonSettings settings)
        {
            ...
            while (parseState.CurrentTokenIndex < parseState.Tokens.Count)
            {
                ...
                if (MemberToDataDictionary.TryGetValue(key, out BMemberData memData))
                {
                    ...
                    if (TypeUtils.IsSubTypeOf(memData.Type, typeof(Component)))
                    {
                        cnv = RefMonoBehaviourConverter;
                    }

                    else if (!settings.HasCustomConverter(memData.Type, out cnv))
                    {
