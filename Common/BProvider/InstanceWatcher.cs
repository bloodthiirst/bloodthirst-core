...
namespace Bloodthirst.Core.BProvider
{
    public class InstanceWatcher<T> : IDisposable
    {
        ...
        internal InstanceWatcher(BProvider provider)
        {
            ...
            TreeLeaf<Type, IBProviderList> leaf = null;

            if (type.IsClass)
            {
                leaf = provider.ClassInstances.GetOrCreateLeaf(typeAsKeyArray);
            }
            else
            {
                leaf = provider.InterfaceInstances.GetOrCreateLeaf(typeAsKeyArray);
            }
